...
pub struct World(HashSet<(i64, i64, i64)>);
...
impl World {
    ...
    pub fn parse<R: BufRead>(r: &mut R) -> io::Result<Option<Self>> {
        ...
        for (i, l) in r.lines().enumerate() {
            for (j, c) in l?.chars().enumerate() {
                match c {
                    '.' => { },
                    '#' => { world.insert((i as i64, j as i64, 0)); },
                    _ => return Ok(None),
                };
            }
        }
    ...
    #[inline]
    pub fn is_live(&self, xyz: (i64, i64, i64)) -> bool {
        self.0.contains(&xyz)
    }
...
fn neighbors(xyz: (i64, i64, i64)) -> impl Iterator<Item=(i64, i64, i64)> {
    let (x, y, z) = xyz;
    (x - 1 ..= x + 1).flat_map(move |i| {
        (y - 1 ..= y + 1).flat_map(move |j| {
            (z - 1 ..= z + 1).filter_map(move |k| {
                if (i, j, k) == (x, y, z) {
                    None
                } else {
                    Some((i, j, k))
