...
fn main() {
    ...
    let moons_init = moons.clone();

    let mut x_cycle = None;
    let mut y_cycle = None;
    let mut z_cycle = None;

    let mut seen_x = HashSet::<Vec<(i64, i64)>>::new();
    let mut seen_y = HashSet::<Vec<(i64, i64)>>::new();
    let mut seen_z = HashSet::<Vec<(i64, i64)>>::new();
    ...
    loop {
        ...
        if moons.iter().zip(moons_init.iter())
            .all(|(m1, m2)| m1.position.x == m2.position.x
                && m1.velocity.x == m2.velocity.x) {
            x_cycle = Some(iter);
        }

        if moons.iter().zip(moons_init.iter())
            .all(|(m1, m2)| m1.position.y == m2.position.y
                && m1.velocity.y == m2.velocity.y) {
            y_cycle = Some(iter);
        }

        if moons.iter().zip(moons_init.iter())
            .all(|(m1, m2)| m1.position.z == m2.position.z
    ...
    println!("lcm of {}, {}, {}",
          x_cycle.unwrap(), y_cycle.unwrap(), z_cycle.unwrap());
}
